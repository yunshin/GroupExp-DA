...
def main():
    ...
    ##For rpn exp
    for param in model.roi_head.group_models.parameters():
        param.requires_grad = False

    for param in model.dense_head.group_models.parameters():
        param.requires_grad = False
    ...
    for name, param in model.named_parameters():
        ...
        if 'dense_head' not in name and 'norm' in name:
            pdb.set_trace()
        
        
        if hasattr(param, 'track_running_stats'):
            pdb.set_trace()
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.BatchNorm1d) or isinstance(module, nn.BatchNorm2d):
            print(f"{name}: track_running_stats = {module.track_running_stats}")
    ##########################
    '''
    ...
    if args.pretrained_model is not None:
        model.load_params_from_file_train(filename=args.pretrained_model, to_cpu=dist, logger=logger)
    ...
    #if model.dense_head.load_pseudo:
    #    train_func = train_model_st_load
    #else:    
