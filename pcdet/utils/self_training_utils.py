...
def save_pseudo_label_epoch(model, val_loader, rank, leave_pbar, ps_label_dir, cur_epoch):
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        target_batch['pseudo_collection'] = True
        target_batch['cur_epoch'] = cur_epoch
        with torch.no_grad():
...
def save_pseudo_label_batch(input_dict,
    ...
    for b_idx in range(batch_size):
        ...
        if 'pred_boxes' in pred_dicts[b_idx]:
            ...
            if 'group_indicator' in pred_dicts[b_idx]:
                use_group = True
                group_indicator = pred_dicts[b_idx]['group_indicator'].cpu().numpy()
                group_ths = cfg.SELF_TRAIN.GROUP_SCORE_THRESH
            ...
            if cfg.SELF_TRAIN.get('NEG_THRESH', None):
                ...
                if use_group:
                    group_indicator = group_indicator[remain_mask]
            if use_group == False:
                ...
                group_mask = np.where(group_indicator > 0)[0]
                if len(group_mask) > 0:
                    labels_ignore_scores[group_mask] = group_ths[0]
                ...
                if (ignore_mask[group_mask]==False).sum() > 0: #removing detections from the lowest scores
                    ...
                    mask_no_group = (ignore_mask==False) * (group_indicator_mask == False)
                    indices_no_group = np.where(mask_no_group)[0]

                    lowest_scores_indices = indices_no_group[-num_samples_from_groups:]
                    ignore_mask[lowest_scores_indices] = True
