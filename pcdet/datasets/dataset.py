...
class DatasetTemplate(torch_data.Dataset):
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        def align_obj_points(batch_list):

           
            max_points = np.amax(np.array(batch_list['obj_points_max_points']))
            max_boxes = np.amax(np.array(batch_list['num_gt']))
            all_boxes = np.sum(np.array(batch_list['num_gt']))
            batch_size = len(batch_list['obj_points_max_points'])

            padded_point_clouds = np.zeros((batch_size, max_boxes, max_points, 3))
            masks = np.zeros((batch_size, max_boxes, max_points))  # Mask indicating real points

            obj_points_batch = batch_list['obj_points_list']

            obj_cnt = 0
            for idx_frame in range(batch_size):

                obj_points_frame = obj_points_batch[idx_frame]
                
                for idx_obj in range(len(obj_points_frame)):

                    obj_points = obj_points_frame[idx_obj]
                    if len(obj_points) > 50:
                        #print('\n\n in  \n\n')
                        random_indices = np.random.choice(len(obj_points), size=50, replace=False)  # 5 unique indices
                        obj_points = obj_points[random_indices]
                    
                    n_points = len(obj_points)
                    
                    padded_point_clouds[idx_frame, idx_obj, :n_points, :] = obj_points
                    masks[idx_frame, idx_obj, :n_points] = 1  # Mask the actual points
                    obj_cnt += 1
            
            if obj_cnt != all_boxes:
                print('err')
                pdb.set_trace()

            return padded_point_clouds, masks    
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['obj_points_list']:
                    obj_points, obj_points_masks = align_obj_points(data_dict)
                    ret['obj_points'] = obj_points 
                    ret['obj_points_masks'] = obj_points_masks
                elif key in ['obj_points_max_points', 'num_gt']:
                    continue
                else:
