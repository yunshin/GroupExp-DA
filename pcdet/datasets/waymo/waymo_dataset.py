...
class WaymoDataset(DatasetTemplate):
    ...
    def add_obj_points(self, data_dict):
        
        gt_boxes = data_dict['gt_boxes']
        points = data_dict['points']
        
        ori_points = np.copy(points)
        ori_gt = np.copy(gt_boxes)
        ...
        for idx in range(len(gt_boxes)):
            ...
            obj_points, point_masks = get_points_in_box(points[:,:3], gt_box[:7]) 
            obj_points[:,:3] -= gt_box[:3]

            if len(obj_points) > 10000: #random downsampling to avoid out of memory error
                
                indices = np.random.choice(obj_points.shape[0], size=10000, replace=False)  
                obj_points = obj_points[indices]
                
            if len(obj_points) > max_points:
                max_points = len(obj_points)
        ...
        return obj_points_list, max_points
    def __getitem__(self, index):
        ...
        if self.training:
            obj_points, obj_points_max = self.add_obj_points(data_dict)    
            data_dict['obj_points_list'] = obj_points
            data_dict['obj_points_max_points'] = obj_points_max
            data_dict['num_gt'] = len(data_dict['gt_boxes'])
